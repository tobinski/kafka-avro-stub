...
var KafkaAvroStub = module.exports = cip.extend(function(kafkaAvro) {
  ...
  /** @type {Array.<Function>} Consumer "readt" event listener callbacks */
  this._consumerReadyListeners = [];
...
KafkaAvroStub.prototype.stub = function (schemaRegistryFix) {
  ...
  this.kafkaAvroInitsStub = sinon.stub(this.kafkaAvro, 'init', Promise.method(() => {
    schemaRegistryFix.forEach((srItem) => {
      ...
      this.kafkaAvro.sr.valueSchemas[srItem.subject] = type;
      this.kafkaAvro.sr.schemaMeta[srItem.subject] = srItem;
    });
  }));
...
KafkaAvroStub.prototype._getConsumer = Promise.method(function () {
  ...
  var consumer = {
    on: function(eventName, cb) {
      ...
      if (eventName === 'ready') {
        self._consumerReadyListeners.push(cb);
      }
    },
    connect: function(meta, optCb) {
      if (typeof optCb === 'function') {
        optCb(null);
      }

      self._consumerReadyListeners.forEach(function(cb) {
        cb();
      });
    },
